            ...
            return new Promise((resolve, reject) => {
                if (this.config.mathRenderingOption === 'MathJax' || this.config.usePandocParser) {
                    ...
                    window['MathJax'].Hub.Queue(['Typeset', MathJax.Hub, this.hiddenPreviewElement], [() => {
                            ...
                            if (!this.hiddenPreviewElement.getElementsByClassName('MathJax').length)
                                return;
                            this.scrollMap = null;
                            return resolve();
                        }]);
        ...
        bindTagAClickEvent() {
            ...
            const helper = (as) => {
                for (let i = 0; i < as.length; i++) {
                    ...
                    if (href && href[0] === '#') {
                        const targetElement = this.previewElement.querySelector(`[id=\"${href.slice(1)}\"]`); // fix number id bug
                        if (targetElement) {
                            a.onclick = (event) => {
                                event.preventDefault();
                                event.stopPropagation();
