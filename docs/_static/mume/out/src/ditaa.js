...
function render(code = '', args = [], dest = '') {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const info = yield utility.tempOpen({ prefix: 'mume_ditaa', 'suffix': '.ditaa' });
            yield utility.writeFile(info.fd, code);
            if (!dest) {
                const info = yield utility.tempOpen({ prefix: 'mume_ditaa', 'suffix': '.png' });
                dest = info.path;
            }
            if (dest in CACHE && CACHE[dest].code === code && utility.isArrayEqual(args, CACHE[dest].args)) {
                return CACHE[dest].outputDest;
            }
            yield utility.execFile('java', [
                '-Djava.awt.headless=true',
                '-jar', path.resolve(utility.extensionDirectoryPath, './dependencies/ditaa/ditaa.jar'),
                info.path, dest
            ].concat(args));
