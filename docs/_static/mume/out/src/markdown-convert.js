...
function processMath(text, { mathInlineDelimiters, mathBlockDelimiters }) {
    ...
    line = line.replace(new RegExp(`(\`\`\`(?:[\\s\\S]+?)\`\`\`\\s*(?:\\n|$))|(?:${inlineBegin}([\\s\\S]+?)${inlineEnd})`, 'g'), ($0, $1, $2) => {
        ...
        math = math.replace(/\n/g, '').replace(/\#slash\_dollarsign\#/g, '\\\$');
        math = utility.escapeString(math);
        return `<img src=\"https://latex.codecogs.com/gif.latex?${(math.trim().replace(/ /g, '%20'))}\"/>`;
    });
...
function markdownConvert(text, { projectDirectoryPath, fileDirectoryPath, protocolsWhiteListRegExp, filesCache, mathInlineDelimiters, mathBlockDelimiters, codeChunksData, graphsCache, usePandocParser }, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const data = yield transformer_1.transformMarkdown(text, { fileDirectoryPath, projectDirectoryPath, useRelativeFilePath, filesCache, forPreview: false, forMarkdownExport: true, protocolsWhiteListRegExp, imageDirectoryPath, usePandocParser });
        ...
        return yield new Promise((resolve, reject) => {
            mkdirp(imageDirectoryPath, (error, made) => {
                ...
                process_graphs_1.processGraphs(text, { fileDirectoryPath, projectDirectoryPath, imageDirectoryPath, imageFilePrefix: compute_checksum_1.default(outputFilePath), useRelativeFilePath, codeChunksData, graphsCache })
                    .then(({ outputString }) => {
