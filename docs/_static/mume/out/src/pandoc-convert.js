 ...
 */
function pandocConvert(text, { fileDirectoryPath, projectDirectoryPath, sourceFilePath, filesCache, protocolsWhiteListRegExp, /*deleteImages=true,*/ codeChunksData, graphsCache, imageDirectoryPath, pandocMarkdownFlavor, pandocPath, latexEngine }, config = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let data = yield transformer_1.transformMarkdown(text, { fileDirectoryPath, projectDirectoryPath, useRelativeFilePath: true, filesCache, protocolsWhiteListRegExp, forPreview: false, usePandocParser: true });
        ...
        if (tocBracketEnabled) {
            ...
            const { content: tocMarkdown } = toc_1.toc(headings, { ordered: false, depthFrom: 1, depthTo: 6, tab: '\t' });
        ...
        yield utility.mkdirp(imageDirectoryPath); // create imageDirectoryPath
        const { outputString, imagePaths } = yield process_graphs_1.processGraphs(text, { fileDirectoryPath, projectDirectoryPath, imageDirectoryPath, imageFilePrefix: compute_checksum_1.default(outputFilePath), useRelativeFilePath: true, codeChunksData, graphsCache });
        // pandoc will cause error if directory doesn't exist,
        // therefore I will create directory first.
        yield utility.mkdirp(path.dirname(outputFilePath));
